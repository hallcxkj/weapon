...
namespace Unity.FPS.Game
{
   ...
   private float reloadTime = 5f;  // Tempo que o reload vai durar
private float reloadTimer = 0f;  // Temporizador para controlar o tempo de reload
        void Awake()
        {
...
void UpdateAmmo()
{
    if (Input.GetKeyDown(KeyCode.R) && m_CurrentAmmo < MaxAmmo && !IsCharging)
    {
        if (weaponAnimator1)
        {
            weaponAnimator1.SetTrigger("reload2");
        }

        // Recarrega a arma ao longo do tempo
        m_CurrentAmmo = 30;
